...
void setup()
{
  ...
  // Clear the buffer
  display.clearDisplay();
  print_line("Welcome to Medibox", 1, 0, 0,1); // (String, text_size, cursor_row, cursor_column)
  delay(3000);
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    delay(250);
    display.clearDisplay();
    print_line("Connecting to Wifi   " + SSID, 1, 0, 0,1); // (String, text_size, cursor_row, cursor_column)
  }
  delay(2000);
  display.clearDisplay();
